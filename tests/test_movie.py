...
def test_put_rate_movie_status_code_404():
    data='{"value": 8.5}'
    guest_id = get_guest_session_id('3/authentication/guest_session/new', valid_api_key)
    status_code, response_data = put_api('3/movie/{invalid_movie_id}/rating', valid_api_key, guest_id, data)
    assert status_code == 404
    assert response_data['status_message'] == 'The resource you requested could not be found.'